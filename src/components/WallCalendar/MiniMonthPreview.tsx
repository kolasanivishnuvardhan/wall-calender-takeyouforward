import { addMonths, eachDayOfInterval, endOfWeek, format, startOfMonth, startOfWeek } from 'date-fns';
...
function PreviewCard({ offset, title }: PreviewCardProps): JSX.Element {
  ...
  const start: Date = startOfWeek(startOfMonth(month), { weekStartsOn: 1 });
  const days: Date[] = eachDayOfInterval({ start, end: endOfWeek(addMonths(start, 1), { weekStartsOn: 1 }) }).slice(0, 35);
  ...
  return (
    ...
    >
      <p className="mb-1 text-[10px] uppercase tracking-[0.08em] text-slate-500">{title}</p>
      <p className="mb-2 text-sm font-semibold text-slate-700 dark:text-slate-200">{format(month, 'MMMM')}</p>
      <div className="grid grid-cols-7 gap-1 text-[10px] text-slate-400">
        {days.map((day: Date) => (
          <span key={`${offset}-${format(day, 'yyyy-MM-dd')}`} className="flex h-5 items-center justify-center">
            {format(day, 'd')}
          </span>
        ))}
      </div>
