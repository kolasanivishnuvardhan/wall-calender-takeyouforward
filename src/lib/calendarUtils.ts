...
const SUNDAY_WEEK_START = 0;

/** Build a fixed 6-row (42 cell) month grid, Sunday-first. */
export function buildDayGrid(month: Date, options: DayGridOptions): DayData[] {
  ...
  const gridStart: Date = startOfWeek(monthStart, {
    weekStartsOn: SUNDAY_WEEK_START,
  });
  const gridEnd: Date = endOfWeek(addDays(gridStart, 41), {
    weekStartsOn: SUNDAY_WEEK_START,
  });
  ...
  return days.map((date: Date) => {
    ...
    return {
      ...
      isDisabled: Boolean(
        ...
        isBefore(date, options.today) &&
        !isSameDay(date, options.today),
      ),
